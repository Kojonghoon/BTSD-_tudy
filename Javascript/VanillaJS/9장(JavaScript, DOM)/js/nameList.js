function newRegister() {
  ...
  delBtn.appendChild(delText) //텍스트 노드가 button 하위 요소로 연걸하기
  newP.appendChild(delBtn)  //delBtn을 p요소의 하위 요소로 만들기


  var nameList = document.querySelector("#nameList")
  //p요소의 #nameList의 맨 앞에 추가
  nameList.insertBefore(newP, nameList.childNodes[0])
  // nameList.appendChild(newP)  //노드가 #nameList의 하위요소가 된다.
  ...
  for (var i = 0; i < removeBtns.length; i++) {
    ...
    removeBtns[i].addEventListener("click", function () {
      //#nameList를 의미를 하게 되는 것이다. span태그의 부모가 p이고
      //p의 부모가 nameList이기 때문이다.
